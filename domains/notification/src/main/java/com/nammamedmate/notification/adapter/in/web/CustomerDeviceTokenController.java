...
@RestController
@RequestMapping("/api/v1/customers/me/device-token")
@Tag(name = "Customer device tokens")
public class CustomerDeviceTokenController {

  private final DeviceTokenService tokens;

  public CustomerDeviceTokenController(DeviceTokenService tokens) {
    this.tokens = tokens;
  }

  @PostMapping
  @Operation(summary = "Register or refresh FCM device token")
  ...
  public ApiResponse<Map<String, Object>> unregister(
    ...
    return ApiResponse.ok(
        tokens.unregister(principal.subject(), NotificationUserType.CUSTOMER, req.deviceId()));
  }
