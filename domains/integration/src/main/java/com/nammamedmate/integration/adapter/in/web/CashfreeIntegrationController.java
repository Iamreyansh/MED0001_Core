...
@RestController
@RequestMapping("/api/v1/integrations/cashfree")
@Tag(name = "Cashfree integration (S2S)")
public class CashfreeIntegrationController {
  ...
  public CashfreeIntegrationController(
      CashfreeIntegrationService service, InternalServiceAuth internalAuth) {
    this.service = service;
    this.internalAuth = internalAuth;
  }

  @PostMapping("/create-order")
  @Operation(summary = "Create Cashfree order (internal token)")
  ...
  @Operation(summary = "Cashfree webhook (HMAC x-webhook-signature)")
  public ApiResponse<Void> webhook(
      @RequestHeader(value = "x-webhook-signature", required = false) String signature,
      @RequestHeader(value = "x-webhook-timestamp", required = false) String timestamp,
      HttpServletRequest request) {
    service.handleWebhook(signature, timestamp, WebhookRawBodyFilter.rawBody(request));
  ...
  @Operation(summary = "Create CashfreePayout fund account (internal token)")
