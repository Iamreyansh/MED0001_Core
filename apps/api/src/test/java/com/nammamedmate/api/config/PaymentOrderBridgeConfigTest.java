...
class PaymentOrderBridgeConfigTest {
  ...
  @Test
  void bridgesPaymentDomainCashfreeAsPrimaryOrderClient() {
    PaymentOrderBridgeConfig config = new PaymentOrderBridgeConfig();
    com.nammamedmate.payment.application.port.out.CashfreeGatewayPort gateway =
        mock(com.nammamedmate.payment.application.port.out.CashfreeGatewayPort.class);
    ...
    when(gateway.createOrder(eq(orderId), eq(500L)))
        .thenReturn(
            new com.nammamedmate.payment.application.port.out.CashfreeGatewayPort.CreateOrderResult(
                "order_live", 500L, "cf_k"));
    ...
    when(gateway.refund("pay", 100L))
        .thenReturn(
            new com.nammamedmate.payment.application.port.out.CashfreeGatewayPort.RefundResult(
                "rfnd", 100L));
    when(payments.handleWebhook(eq("h"), any())).thenReturn(Map.of("processed", true));
    var port = config.orderCashfreeFromPaymentDomain(gateway, payments);
    assertThat(port.createOrder(orderId, 500L).gatewayOrderId()).isEqualTo("order_live");
    ...
    assertThat(port.refund("pay", 100L).gatewayRefundId()).isEqualTo("rfnd");
