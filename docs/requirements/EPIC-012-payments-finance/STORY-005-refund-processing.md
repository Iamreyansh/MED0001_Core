...
| # | Rule |
|---|---|
| BR-001 | **Refund routing logic:** `if payment_method == COD ? refund_to = WALLET` (instant). `if payment_method == ONLINE and amount <= wallet_portion ? refund_to = WALLET`. `if online payment and gateway_portion > 0 ? refund_to = SOURCE_ACCOUNT` via Razorpay refund API. |
...
| BR-006 | Online payment refunds via Razorpay are expected to reach the customer's source account in **3-5 business days**; the status is updated to `COMPLETED` when Razorpay sends the `refund.processed` webhook. |
...
**Description:** Process a pending refund. Triggers Razorpay refund API (online) or wallet credit (COD/wallet).
...
| `RAZORPAY_REFUND_FAILED` | 502 | Razorpay API returned an error |
...
| `razorpay_refund_id` | VARCHAR(100) | Yes | Razorpay refund reference |
| `razorpay_payment_id` | VARCHAR(100) | Yes | Original payment reference |
...
| AC-004 | When the Razorpay `refund.processed` webhook arrives, the refund status is updated to `COMPLETED`; the customer receives a push notification. |
...
| AC-008 | For a hybrid payment (Rs 50 wallet + Rs 445 UPI), the refund for full amount credits Rs 50 to wallet immediately and initiates a Rs 445 Razorpay refund. |
...
|---|---|---|
| Payment Processing (EPIC-012/STORY-001) | Internal | Razorpay payment_id used for refund API call |
...
| Razorpay Refund API | External | Online payment refund disbursement |
| Razorpay Webhook (EPIC-012/STORY-001) | External | `refund.processed` event updates status to COMPLETED |
...
- For UPI and card refunds, the actual credit to the customer's account is handled by the bank/UPI network after Razorpay processes the refund; Namma MedMate has no control over the final delivery timing.
- When a Razorpay refund fails, the `Refund` status is set to `FAILED`; admin_finance is alerted and must investigate the Razorpay dashboard.
